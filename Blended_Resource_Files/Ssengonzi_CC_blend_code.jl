...
csvimport2 = CSV.read("All_NonVRE_and_hydro_generators_2018_abridged.csv", DataFrame) # Generator specifications for non-VREs and hydro plants
...
a_series = DataFrame() # CA New wind, representative 
b_series = DataFrame() # CA New solar
c_series = DataFrame() # CA New offshore wind , won't be used
d_series = DataFrame() # NW New wind 
e_series = DataFrame() # NW New solar
f_series = DataFrame() # NW New offshore wind , won't be used
...
u_series = DataFrame() # NE New offshore wind , won't be used
...
CA_rep_clus_names = []
CEN_rep_clus_names = []
MID_AT_rep_clus_names = []
NE_rep_clus_names = []
NW_rep_clus_names = []
N_CEN_rep_clus_names = []
SE_rep_clus_names = []
SW_rep_clus_names = []
TX_rep_clus_names = []

colname ="Reg_rep"
a_series_rep = DataFrame() # CA New wind, representative 
a_series_rep[!,colname] = a_series[!,3]
b_series_rep = DataFrame() # CA New solar
b_series_rep[!,colname] = b_series[!,4]
#c_series_rep = DataFrame() # CA New offshore wind , won't be used

d_series_rep = DataFrame() # NW New wind 
d_series_rep[!,colname] = d_series[!,12]
e_series_rep = DataFrame() # NW New solar
e_series_rep[!,colname] = e_series[!,3]
#f_series_rep = DataFrame() # NW New offshore wind , won't be used

g_series_rep = DataFrame() # SW New wind
g_series_rep[!,colname] = g_series[!,7]
h_series_rep = DataFrame() # SW New solar
h_series_rep[!,colname] = h_series[!,17]

i_series_rep = DataFrame() # TX New wind
i_series_rep[!,colname] = i_series[!,3]
j_series_rep = DataFrame() # TX New solar
j_series_rep[!,colname] = j_series[!,2]

k_series_rep = DataFrame() # N_CEN New wind
k_series_rep[!,colname] = k_series[!,25]
l_series_rep = DataFrame() # N_CEN New solar
l_series_rep[!,colname] = l_series[!,33]

m_series_rep = DataFrame() # CEN New wind
m_series_rep[!,colname] = m_series[!,26]
n_series_rep = DataFrame() # CEN New solar
n_series_rep[!,colname] = n_series[!,24]

o_series_rep = DataFrame() # SE New wind
o_series_rep[!,colname] = o_series[!,31]
p_series_rep = DataFrame() # SE New solar
p_series_rep[!,colname] = p_series[!,32]

q_series_rep = DataFrame() # MID_AT New wind
q_series_rep[!,colname] = q_series[!,26]
r_series_rep = DataFrame() # MID_AT New solar
r_series_rep[!,colname] = r_series[!,26]

s_series_rep = DataFrame() # NE New wind
s_series_rep[!,colname] = s_series[!,2]
t_series_rep = DataFrame() # NE New solar
t_series_rep[!,colname] = t_series[!,1]

#u_series_rep = DataFrame() # NE New offshore wind , won't be used

#a_series_rep[!,colname] = a_series[!,i]

function additional_cluster_names(region_name, clus_name_array)
    append!(clus_name_array,[region_name*"_bl_0_100"])
    append!(clus_name_array,[region_name*"_bl_10_90"])
    append!(clus_name_array,[region_name*"_bl_20_80"])
    append!(clus_name_array,[region_name*"_bl_30_70"])
    append!(clus_name_array,[region_name*"_bl_40_60"])
    append!(clus_name_array,[region_name*"_bl_50_50"])
    append!(clus_name_array,[region_name*"_bl_60_40"])
    append!(clus_name_array,[region_name*"_bl_70_30"])
    append!(clus_name_array,[region_name*"_bl_80_20"])
    append!(clus_name_array,[region_name*"_bl_90_10"])
    append!(clus_name_array,[region_name*"_bl_100_0"])

end

additional_cluster_names(CA_name, CA_rep_clus_names)
additional_cluster_names(CEN_name, CEN_rep_clus_names)
additional_cluster_names(MID_AT_name, MID_AT_rep_clus_names)
additional_cluster_names(NE_name, NE_rep_clus_names)
additional_cluster_names(NW_name, NW_rep_clus_names)
additional_cluster_names(N_CEN_name, N_CEN_rep_clus_names)
additional_cluster_names(SE_name, SE_rep_clus_names)
additional_cluster_names(SW_name, SW_rep_clus_names)
additional_cluster_names(TX_name, TX_rep_clus_names)
...
function existing_vre_specs(reg_solar_cap, reg_on_wind_cap, reg_solar_CF, reg_on_wind_CF)
...
function new_vre_specs(incr_size, WND_series, PV_series)
...
function gen_raw_blend(exist_gen_raw,grad_cap,wind_series,solar_series,wind_reg_count,solar_reg_count,decimal_portion,reg_demand,clus_name_subset)
...
function full_CC_operation(RN,OT,OV,clus_name_array,region_name,gen_specs, reg_demand, reg_capacities, reg_on_wind_CF, reg_solar_CF, incr_size, WND_series, PV_series, reg_counts,x_cap_frame)
    ...
    function base_case()
        ...
        existing_vre_specs(reg_solar_cap,reg_on_wind_cap, reg_solar_CF, reg_on_wind_CF) 
    ...
    new_vre_specs(incr_size, WND_series, PV_series)
    ...
    bl_0_100 = PV_gen[:,(reg_counts[2])]
    bl_10_90 = 0.1 * WND_gen[:,(reg_counts[1])] + 0.9 * PV_gen[:,(reg_counts[2])]
    bl_20_80 = 0.2 * WND_gen[:,(reg_counts[1])] + 0.8 * PV_gen[:,(reg_counts[2])]
    bl_30_70 = 0.3 * WND_gen[:,(reg_counts[1])] + 0.7 * PV_gen[:,(reg_counts[2])]
    bl_40_60 = 0.4 * WND_gen[:,(reg_counts[1])] + 0.6 * PV_gen[:,(reg_counts[2])]
    bl_50_50 = 0.5 * WND_gen[:,(reg_counts[1])] + 0.5 * PV_gen[:,(reg_counts[2])]
    bl_60_40 = 0.6 * WND_gen[:,(reg_counts[1])] + 0.4 * PV_gen[:,(reg_counts[2])]
    bl_70_30 = 0.7 * WND_gen[:,(reg_counts[1])] + 0.3 * PV_gen[:,(reg_counts[2])]
    bl_80_20 = 0.8 * WND_gen[:,(reg_counts[1])] + 0.2 * PV_gen[:,(reg_counts[2])]
    bl_90_10 = 0.9 * WND_gen[:,(reg_counts[1])] + 0.1 * PV_gen[:,(reg_counts[2])]
    bl_100_0 = WND_gen[:,(reg_counts[1])]

    global k = 1
    cluster_op(bl_0_100, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_10_90, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_20_80, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_30_70, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_40_60, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_50_50, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_60_40, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_70_30, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_80_20, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_90_10, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    cluster_op(bl_100_0, clus_name_array, region_name, (0.5*incr_size[1] + 0.5*incr_size[2]))
    global k += 1
    ...
    blend_variance_range = [1:11;]

    gen_raw_blend(bl_0_100,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0,1], reg_demand, clus_name_array[blend_variance_range[1]]) 
    gen_raw_blend(bl_10_90,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.1,0.9], reg_demand, clus_name_array[blend_variance_range[2]])
    gen_raw_blend(bl_20_80,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.2,0.8], reg_demand, clus_name_array[blend_variance_range[3]]) 
    gen_raw_blend(bl_30_70,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.3,0.7], reg_demand, clus_name_array[blend_variance_range[4]])
    gen_raw_blend(bl_40_60,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.4,0.6], reg_demand, clus_name_array[blend_variance_range[5]]) 
    gen_raw_blend(bl_50_50,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.5,0.5], reg_demand, clus_name_array[blend_variance_range[6]]) 
    gen_raw_blend(bl_60_40,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.6,0.4], reg_demand, clus_name_array[blend_variance_range[7]])
    gen_raw_blend(bl_70_30,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.7,0.3], reg_demand, clus_name_array[blend_variance_range[8]])
    gen_raw_blend(bl_80_20,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.8,0.2], reg_demand, clus_name_array[blend_variance_range[9]])
    gen_raw_blend(bl_90_10,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[0.9,0.1], reg_demand, clus_name_array[blend_variance_range[10]])
    gen_raw_blend(bl_100_0,wind_solar_grad_cap,WND_series,PV_series,reg_counts[1],reg_counts[2],[1,0], reg_demand, clus_name_array[blend_variance_range[11]])


    #-----------------------------------------------
    # Output for capacity levels
    
    for i in 1:11
        x_cap_frame[!,clus_name_array[i]] = wind_solar_grad_cap_rec
    end

    CSV.write("PLT_28_bl_1 $region_name Capacity_levels_df.csv", x_cap_frame)
...
global res_incr_size = [1000,1000] # resource increment in MW size for on_wind_incr size, solar_incr size, off_wind_incr size
...
df_reset()
CA_time = @elapsed full_CC_operation(RN,OT,OV,CA_rep_clus_names,CA_name, CA_gen_specs, CA_demand, CA_capacities, E_WND_CA, E_PV_CA, res_incr_size, a_series, b_series, reg_count_univ, CA_x_cap_frame)
CA_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_CA_Penetration_levels_df.csv", CA_x_pen_frame)
CA_y_frame = C_Creds
CSV.write("PLT_28_bl_1_CA_Cap_credits_df.csv", CA_y_frame)


global num_of_incr = num_incr_param[2] # number of times to add resource to grid
df_reset()
NW_time = @elapsed full_CC_operation(RN,OT,OV,NW_rep_clus_names,NW_name, NW_gen_specs, NW_demand, NW_capacities, E_WND_NW, E_PV_NW, res_incr_size, d_series, e_series, reg_count_univ, NW_x_cap_frame)
NW_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_NW_Penetration_levels_df.csv", NW_x_pen_frame)
NW_y_frame = C_Creds
CSV.write("PLT_28_bl_1_NW_Cap_credits_df.csv", NW_y_frame)


global num_of_incr = num_incr_param[3] # number of times to add resource to grid
df_reset()
SW_time = @elapsed full_CC_operation(RN,OT,OV,SW_rep_clus_names,SW_name, SW_gen_specs, SW_demand, SW_capacities, E_WND_SW, E_PV_SW, res_incr_size, g_series, h_series, reg_count_univ, SW_x_cap_frame)
SW_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_SW_Penetration_levels_df.csv", SW_x_pen_frame)
SW_y_frame = C_Creds
CSV.write("PLT_28_bl_1_SW_Cap_credits_df.csv", SW_y_frame)

global num_of_incr = num_incr_param[4] # number of times to add resource to grid
df_reset()
TX_time = @elapsed full_CC_operation(RN,OT,OV,TX_rep_clus_names,TX_name, TX_gen_specs, TX_demand, TX_capacities, E_WND_TX, E_PV_TX, res_incr_size, i_series, j_series, reg_count_univ, TX_x_cap_frame)
TX_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_TX_Penetration_levels_df.csv", TX_x_pen_frame)
TX_y_frame = C_Creds
CSV.write("PLT_28_bl_1_TX_Cap_credits_df.csv", TX_y_frame)

global num_of_incr = num_incr_param[5] # number of times to add resource to grid
df_reset()
N_CEN_time = @elapsed full_CC_operation(RN,OT,OV,N_CEN_rep_clus_names,N_CEN_name, N_CEN_gen_specs, N_CEN_demand, N_CEN_capacities, E_WND_N_CEN, E_PV_N_CEN, res_incr_size, k_series, l_series, reg_count_univ, N_CEN_x_cap_frame)
N_CEN_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_N_CEN_Penetration_levels_df.csv", N_CEN_x_pen_frame)
N_CEN_y_frame = C_Creds
CSV.write("PLT_28_bl_1_N_CEN_Cap_credits_df.csv", N_CEN_y_frame)

global num_of_incr = num_incr_param[6] # number of times to add resource to grid
df_reset()
CEN_time = @elapsed full_CC_operation(RN,OT,OV,CEN_rep_clus_names,CEN_name, CEN_gen_specs, CEN_demand, CEN_capacities, E_WND_CEN, E_PV_CEN, res_incr_size, m_series, n_series, reg_count_univ, CEN_x_cap_frame)
CEN_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_CEN_Penetration_levels_df.csv", CEN_x_pen_frame)
CEN_y_frame = C_Creds
CSV.write("PLT_28_bl_1_CEN_Cap_credits_df.csv", CEN_y_frame)

global num_of_incr = num_incr_param[7] # number of times to add resource to grid
df_reset()
SE_time = @elapsed full_CC_operation(RN,OT,OV,SE_rep_clus_names,SE_name, SE_gen_specs, SE_demand, SE_capacities, E_WND_SE, E_PV_SE, res_incr_size, o_series, p_series, reg_count_univ, SE_x_cap_frame)
SE_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_SE_Penetration_levels_df.csv", SE_x_pen_frame)
SE_y_frame = C_Creds
CSV.write("PLT_28_bl_1_SE_Cap_credits_df.csv", SE_y_frame)

global num_of_incr = num_incr_param[8] # number of times to add resource to grid
df_reset()
MID_AT_time = @elapsed full_CC_operation(RN,OT,OV,MID_AT_rep_clus_names,MID_AT_name, MID_AT_gen_specs, MID_AT_demand, MID_AT_capacities, E_WND_MID_AT, E_PV_MID_AT, res_incr_size, q_series, r_series, reg_count_univ, MID_AT_x_cap_frame)
MID_AT_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_MID_AT_Penetration_levels_df.csv", MID_AT_x_pen_frame)
MID_AT_y_frame = C_Creds
CSV.write("PLT_28_bl_1_MID_AT_Cap_credits_df.csv", MID_AT_y_frame)

global num_of_incr = num_incr_param[9] # number of times to add resource to grid
df_reset()
NE_time = @elapsed full_CC_operation(RN,OT,OV,NE_rep_clus_names,NE_name, NE_gen_specs, NE_demand, NE_capacities, E_WND_NE, E_PV_NE, res_incr_size, s_series, t_series, reg_count_univ, NE_x_cap_frame)
NE_x_pen_frame = pen_levels
CSV.write("PLT_28_bl_1_NE_Penetration_levels_df.csv", NE_x_pen_frame)
NE_y_frame = C_Creds
CSV.write("PLT_28_bl_1_NE_Cap_credits_df.csv", NE_y_frame)
...
CSV.write("PLT_28_bl_All_regions_runtime.csv", runtime_results_df)

println(results_df) #Will print out database of results for regions
CSV.write("PLT_28_bl_All_regions.csv", results_df)
